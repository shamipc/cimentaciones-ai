...
with r3:
    st.session_state.h_min, st.session_state.h_max = st.slider(
        "Altura h (m)", 0.3, 2.5, (float(st.session_state.h_min), float(st.session_state.h_max))
    )
...
if st.button("🚀 Analizar soluciones", use_container_width=True):
    ...
    texto = (
        ...
        f"margen = **{mejor['margen']:.1f} kPa**  \n"
        f"- Costo estimado = **S/ {mejor['costo']:.2f}**  \n\n"
        "**Opción robusta (≤ P25 de costo, mayor margen)**  \n"
        f"- B = **{robusta['B']:.2f} m**, L = **{robusta['L']:.2f} m**, h = **{robusta['h']:.2f} m**  \n"
    ...
    csv_sol = df_ok.sort_values("costo").to_csv(index=False)
    st.download_button(
